...
记忆会随时间自然衰减，除非被频繁使用而强化。每个实体和关系都有一个**强度**分数，按指数衰减——经常访问的记忆保持鲜明，未使用的逐渐褪色并最终过期。

- **视觉衰减** — 节点透明度反映记忆强度，褪色节点表示正在老化的知识
- **"显示衰减"模式** — 按强度为图谱着色（绿色 → 黄色 → 红色）
- **时间旅行** — 拖动滑块查看图谱在任意时间点的状态。尚未创建的节点消失，已过期的节点以幽灵形式重新出现。点击**自动播放**，观看知识图谱从头开始生长。
- **恢复** — 一键恢复任何已过期的实体
...
### MCP 服务器 — 与 Claude Code 和 Cursor 配合使用

MindReader 包含一个 [MCP 服务器](https://github.com/flu012/mindreader-mcp)，可与 Claude Code、Cursor 或任何兼容 MCP 的客户端集成。六个工具：搜索、存储、创建、回忆、列出实体和统计。
...
支持批量创建、更新插入和关系管理。详见 [API 参考文档](docs/api-direct-entity.md)。
